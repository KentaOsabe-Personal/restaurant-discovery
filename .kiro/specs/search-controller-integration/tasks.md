...
- [x] 2. `create` アクションにエンドツーエンドの検索フローを実装する
- [x] 2.1 QueryParserService を呼び出して自然文を構造化条件に変換する
  - 既存のバリデーション（空文字・型チェック）を維持したまま、その後に QueryParserService を呼び出す
  - 戻り値の構造化条件（area / genre / price_level / keyword）を後続処理に引き渡す
  - _Requirements: 1.1, 1.5_

- [x] 2.2 GooglePlacesService を呼び出して候補店リストを取得し、空の場合は早期リターンする
...
- [x] 2.3 RecommendationService を呼び出してレスポンスを組み立てる
...
- [x] 3. request spec を統合フロー向けに更新する
- [x] 3.1 (P) 正常系テストを 3 サービスモック付きで実装する
