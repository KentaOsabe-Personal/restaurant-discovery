...
- [x] 1. App コンポーネントに状態管理と検索フローを実装する (P)
- [x] 1.1 (P) アプリケーション状態（isLoading / recommendations / error）を useState で定義し、初期値を設定する
...
- [x] 1.2 (P) handleSearch 関数を実装し、検索ライフサイクル（開始・成功・エラー）を制御する
...
- [x] 1.3 SearchInput・タイトル・各レンダリング条件を JSX に組み込む
...
- [x] 2. RecommendationList コンポーネントを実装する (P)
- [x] 2.1 (P) Recommendation 配列を受け取り PlaceCard のリストを `<ul>/<li>` 構造でレンダリングする
...
- [x] 3. App コンポーネントの統合テストを App.test.tsx に記述する
- [x] 3.1 成功シナリオと空状態シナリオのテストを実装する
...
- [x] 3.4 全テストが pnpm test --run でパスすることを確認する
